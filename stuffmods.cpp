...
static void cgmHook(void* self){
    ...
    if(!g_hudBuilt&&g_frame==60){
        LOG("attempting buildHUD...");
        g_hudBuilt=buildHUD(self);
        LOG("buildHUD result=%d",g_hudBuilt);
    }
    if(g_hudBuilt&&g_frame%30==0) updateHUD();
    if(!g_hudBuilt&&g_frame%300==1) LOG("HUD not built frame=%d",g_frame);
}
        ...
        if(dom){
            ...
            if(_getAsms){
                ...
                for(size_t a=0;a<asmCount;a++){
                    ...
                    void* cls=_classFromName(img,"TMPro","TextMeshPro");
                    if(cls){_textMeshClass=cls;LOG("TextMeshPro found in asm index %zu",(size_t)a);}
                    void* mrCls=_classFromName(img,"","MeshRenderer"); if(!mrCls) mrCls=_classFromName(img,"UnityEngine","MeshRenderer");
